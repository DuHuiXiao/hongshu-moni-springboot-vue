<template>
          ...
          <div class="info">
            ...
            <div class="data-info">
              <div class="user-interactions">
                ...
                <div @click="getUserFollowers" class="follower">
  ...
  <ElDialog v-model:model-value="friendShow" width="300px">
    <template #header>关注列表</template>
    <ul>
      <li v-for="(item) in friends" :key="item.uid">
        <div class="item-div" @click="toUser(item.uid)">
          <ElAvatar :src="item.avatar"></ElAvatar>
...
import { followById, getFriendPage, isFollow } from "@/api/follower";
import { useRoute } from "vue-router";
import { ElAvatar, ElDialog, ElMessage } from "element-plus";
...
const page = ref(1) //页码
const limit = 10000 //每页条数
const friends = ref<any>([])
async function getFriends(){
  const res = await getFriendPage(page.value,limit)
  friends.value = friends.value.concat(res.data.records);
  console.log('page',friends.value);
...
const toUser = (uid:string)=>{
  router.push({name:'user',query:{uid}})
}
